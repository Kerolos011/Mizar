...
export function toEnglishDigits(value: any) {
  return String(value ?? "")
...
export function money(value: any, propsOrStore: any, locale: Locale) {
  ...
  return locale === "ar"
    ? `${toEnglishDigits(formatted)} ${currencyLabel(currency, locale)}`
    : `${currencyLabel(currency, locale)} ${toEnglishDigits(formatted)}`;
}
