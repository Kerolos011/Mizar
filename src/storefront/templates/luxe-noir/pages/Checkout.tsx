...
export function Checkout(props: TemplatePageProps) {
  ...
  return (
    <main className={styles.root} dir={locale === "ar" ? "rtl" : "ltr"} lang={locale} data-template={theme.key}>
      ...
      <section className={styles.pageHero}><div className={styles.shell}><div className={styles.eyebrow}>{theme.name}</div><h1 className={styles.pageTitle}>{locale === "ar" ? "إتمام الطلب" : "Checkout"}</h1><p className={styles.sectionSubtitle}>{locale === "ar" ? "هذه واجهة القالب الخاصة بالدفع، وسيتم ربطها بمنطق الدفع الحالي في المرحلة التالية." : "This is the template checkout surface. It will be connected to the existing checkout logic in the next phase."}</p></div></section>
