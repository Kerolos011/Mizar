...
export function Wishlist(props: TemplatePageProps) {
  ...
  return (
    <main className={styles.root} dir={locale === "ar" ? "rtl" : "ltr"} lang={locale} data-template={theme.key}>
      ...
      <section className={styles.pageHero}><div className={styles.shell}><span className={styles.kicker}>{theme.name}</span><h1 className={styles.pageTitle}>{text.pages.wishlistTitle}</h1><p className={styles.sectionSubtitle}>{text.pages.wishlistSubtitle}</p></div></section>
